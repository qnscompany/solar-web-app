...
type BillRange = 'under3' | '3to6' | '6to10' | 'over10';
type Location = 'house' | 'veranda' | 'factory';

export default function SolarCalculator() {
    const [bill, setBill] = useState<BillRange>('3to6');
    ...
    const calculateResult = () => {
        ...
        if (bill === '3to6' && loc === 'house') {
            return { cap: '5kW', saving: '월 5만원' };
        }
        if (bill === '6to10' && (loc === 'house' || loc === 'factory')) {
            return { cap: '10kW', saving: '월 8만원' };
        }
        if (bill === 'over10' && loc === 'factory') {
            return { cap: '30kW 이상', saving: '상담 필요', specialized: true };
        }
        // [Fallback] 나머지 케이스에 대한 기본값
        return { cap: '3kW', saving: '월 3만원' };
    };
